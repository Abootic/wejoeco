...
class SupplierBloc extends Bloc<SupplierEvent, SupplierState> {
  ...
  SupplierBloc({required this.repository}) : super(SupplierState()) {
  ...
  Future<void> _onLoadDataById(LoadDataById event, Emitter<SupplierState> emit) async {
    ...
    try {
      ...
      if (res != null) {
        print("Supplier data loaded: ${res.marketId}"); // Debug: Verify loaded data
        emit(state.copyWith(
          currentState: StateTypes.loaded,
          error: null,
...
class SupplierState {
  ...
  final String? error;
  final List<SupplierDTO>? data; // List of suppliers
  final SupplierDTO? postdata; // Single supplier
